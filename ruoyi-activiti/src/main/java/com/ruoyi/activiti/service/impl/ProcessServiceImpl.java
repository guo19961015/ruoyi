...
public class ProcessServiceImpl implements IProcessService {
    ...
    @Autowired
    private IPlServiceAgencyItemService iPlServiceAgencyItemService;
    ...
    @Override
    public ProcessInstance submitApplyPl(String applyUserId, String businessKey, String title, String legal, String module, Map<String, Object> variables) {
        // 用来设置启动流程的人员ID，引擎会自动把用户ID保存到activiti:initiator中
        identityService.setAuthenticatedUserId(applyUserId);
        // 启动流程时设置业务 key
        ProcessInstance instance = runtimeService.startProcessInstanceByKey(module, businessKey, variables);
        ...
        plTodoItemService.insertTodoItem2(instance.getProcessInstanceId(), title, legal, module);
        return instance;
    }
    @Override
    public ProcessInstance submitApplyPlate(String applyUserId, String businessKey, String title, String legal, String module, Map<String, Object> variables) {
        // 用来设置启动流程的人员ID，引擎会自动把用户ID保存到activiti:initiator中
        identityService.setAuthenticatedUserId(applyUserId);
        // 启动流程时设置业务 key
        ProcessInstance instance = runtimeService.startProcessInstanceByKey(module, businessKey, variables);
        ...
        plTodoItemService.insertTodoItem3(instance.getProcessInstanceId(), title, legal, module);
        return instance;
    }
    @Override
    ...
    @Override
    public void completePl(Date realityStartTime, Date realityEndTime, BigDecimal actualLoanAmount, String taskId, String instanceId, String itemName, String itemContent, String module, Map<String, Object> variables, HttpServletRequest request) {
        Enumeration<String> parameterNames = request.getParameterNames();
        String comment = null;          // 批注
        boolean agree = true;
        try {
            while (parameterNames.hasMoreElements()) {
                ...
                if (parameterName.startsWith("p_")) {
                    ...
                    if (parameter.length == 3) {
                        String paramValue = request.getParameter(parameterName);
                        Object value = paramValue;
                        if (parameter[1].equals("B") && !paramValue.equals("")) {
                            value = BooleanUtils.toBoolean(paramValue);
                            agree = (boolean) value;
                        } else if (parameter[1].equals("DT") && !paramValue.equals("")) {
                            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
                            value = sdf.parse(paramValue);
                        } else if (parameter[1].equals("COM") && !paramValue.equals("")) {
                            comment = paramValue;
                        }
                        variables.put(parameter[2], value);
                    } else {
            ...
            // 更新待办事项状态
            PlTodoItem query = new PlTodoItem();
            query.setTaskId(taskId);
            // 考虑到候选用户组，会有多个 todoitem 办理同个 task
            List<PlTodoItem> updateList = CollectionUtils.isEmpty(plTodoItemService.selectPlTodoItemList(query)) ? null : plTodoItemService.selectPlTodoItemList(query);
            for (PlTodoItem update: updateList) {
                // 找到当前登录用户的 todoitem，置为已办
                if (update.getTodoUserId().equals(ShiroUtils.getLoginName())) {
                    update.setIsView("1");
                    update.setIsHandle("1");
                    ...
                    update.setHandleUserName(ShiroUtils.getSysUser().getUserName());
                    update.setHandleTime(DateUtils.getNowDate());
                    plTodoItemService.updatePlTodoItem(update);
                } else {
                    plTodoItemService.deletePlTodoItemById(update.getId()); // 删除候选用户组其他 todoitem
                }
            }

            // 下一节点处理人待办事项
            plTodoItemService.insertTodoItem(realityStartTime,realityEndTime,actualLoanAmount,instanceId, itemName, itemContent, module);
        } catch (Exception e) {
            logger.error("error on complete task {}, variables={}", new Object[]{taskId, variables, e});
        }
    }
    @Override
    public void completePlate(String taskId, String instanceId, String itemName, String itemContent, String module, Map<String, Object> variables, HttpServletRequest request) {
        Enumeration<String> parameterNames = request.getParameterNames();
        String comment = null;          // 批注
        boolean agree = true;
        try {
            ...
            if (StringUtils.isNotEmpty(comment)) {
                identityService.setAuthenticatedUserId(ShiroUtils.getLoginName());
                comment = agree ? "【已处理】" + comment : "【未处理】" + comment;
                taskService.addComment(taskId, instanceId, comment);
            }
            // 被委派人处理完成任务
            // p.s. 被委托的流程需要先 resolved 这个任务再提交。
            ...
            // 更新待办事项状态
            PlServiceAgencyItem query = new PlServiceAgencyItem();
            query.setTaskId(taskId);
            // 考虑到候选用户组，会有多个 todoitem 办理同个 task
            List<PlServiceAgencyItem> updateList = CollectionUtils.isEmpty(iPlServiceAgencyItemService.selectPlServiceAgencyItemList(query)) ? null : iPlServiceAgencyItemService.selectPlServiceAgencyItemList(query);
            for (PlServiceAgencyItem update: updateList) {
                // 找到当前登录用户的 todoitem，置为已办
                if (update.getTodoUserId().equals(ShiroUtils.getLoginName())) {
                    update.setIsView("1");
                    update.setIsHandle("1");
                    ...
                    update.setHandleUserName(ShiroUtils.getSysUser().getUserName());
                    update.setHandleTime(DateUtils.getNowDate());
                    iPlServiceAgencyItemService.updatePlServiceAgencyItem(update);
                } else {
                    iPlServiceAgencyItemService.deletePlServiceAgencyItemById(update.getId()); // 删除候选用户组其他 todoitem
                }
            }

            // 下一节点处理人待办事项
            iPlServiceAgencyItemService.insertTodoItem(instanceId, itemName, itemContent, module);
        } catch (Exception e) {
            logger.error("error on complete task {}, variables={}", new Object[]{taskId, variables, e});
        }
    }
