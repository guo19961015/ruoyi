...
//                        .singleResult();
     ...
     * 查询待办任务
     */
    public Page<PlServiceAgencyLoanVo> findPlateTasks(PlServiceAgencyLoanVo leave, String userId) {
        // 手动分页
        PageDomain pageDomain = TableSupport.buildPageRequest();
        Integer pageNum = pageDomain.getPageNum();
        Integer pageSize = pageDomain.getPageSize();
        ...
        for (Task task : tasks) {
            ...
            // 进行数据比对，如果该数据存在表中，则显示，不存在不计入
            int i = 0;
            PlServiceAgencyLoan plServiceAgencyLoan = new PlServiceAgencyLoan();
            Long userId1 = ShiroUtils.getUserId();
            plServiceAgencyLoan.setUserId(userId1);

            String serviceProducts = plServiceAgencyLoanMapper.selectProductIdList(String.valueOf(userId1));
            if (serviceProducts != null) {
                plServiceAgencyLoan.setProductIdList(Convert.toStrArray(serviceProducts));
            }
            List<PlServiceAgencyLoan> plServiceAgencyLoans = plServiceAgencyLoanMapper.selectPlServiceAgencyLoanList(plServiceAgencyLoan);
            plServiceAgencyLoans.removeAll(Collections.singleton(null));
            if (plServiceAgencyLoans != null && leave2 !=null) {
                for (PlServiceAgencyLoan serviceAgencyLoan : plServiceAgencyLoans) {
                    if(serviceAgencyLoan.getId().equals(leave2.getId())){
                        i++;
                    }
                }
            }
