...
public class SysUserController extends BaseController
{
    ...
    @ResponseBody
    public TableDataInfo serviceList(SysUser user)
    {
        startPage();
        user.setStatus("4");
        List<SysUser> list = userService.selectUserList(user);
    ...
    public String approval(@PathVariable("userId") Long userId, ModelMap mmap)
    {
        SysUser sysUser = userService.selectUserById(userId);
        SysDictData sysDictData = new SysDictData();
        sysDictData.setDictType("plate");
        List<SysDictData> sysDictList = iSysDictDataService.selectDictDataList(sysDictData);
        if (sysDictList != null) {
            if (sysDictData != null) {
                String serviceProducts = sysUser.getServiceProducts();
                if (serviceProducts != null) {
                    String[] split = serviceProducts.split(",");
                    if (split.length>0) {
                        for (SysDictData dictData : sysDictList) {
                            for (String s : split) {
        ...
        if (sysUser != null) {
            ...
            ryProduct.setUserId(String.valueOf(sysUser.getUserId()));
            List<RyProduct> ryProducts = iRyProductService.selectRyProductList(ryProduct);
            mmap.put("ryProducts", ryProducts);
        }
        mmap.put("user", sysUser);
        mmap.put("sysDictList", sysDictList);
    ...
    public String add(ModelMap mmap)
    {
        ...
        mmap.put("banks", plBankService.selectPlBankList(new PlBank()));
        mmap.put("plGuarantees", plGuaranteeService.selectPlGuaranteeListDouble(new PlGuarantee()));
        return prefix + "/add";
    }
    ...
    public String edit(@PathVariable("userId") Long userId, ModelMap mmap)
    {
        ...
        plBank.setUserId(String.valueOf(userId));
        mmap.put("banks", plBankService.selectPlBankList(plBank));
        PlGuarantee plGuarantee = new PlGuarantee();
        plGuarantee.setUserId(String.valueOf(userId));
        mmap.put("plGuarantees", plGuaranteeService.selectPlGuaranteeListDouble(plGuarantee));
        return prefix + "/edit";
    }
